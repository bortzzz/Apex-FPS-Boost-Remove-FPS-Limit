# MAKE SURE TO PUT THIS FILE IN APEX FOLDER
# EXAMPLE : D:\Program Files (x86)\Steam\steamapps\common\Apex Legends
setting_apex = """"VideoConfig"
{
...
apex_setting =os.path.expanduser("~") + r"\Saved Games\Respawn\Apex\local"
apex_exe = os.path.join(os.getcwd(), "r5apex.exe")
apex_cfg = os.getcwd() +"\cfg"
verf_cfg = apex_cfg + "\client"
ps1= os.path.join(os.getcwd(), "unwire.ps1")
autoexecNAME = os.path.join(apex_cfg, "autoexec.cfg")
autoexecNAME_verf = os.path.join(verf_cfg, "autoexec.cfg")
save_path = apex_setting
file_name = "videoconfig.txt"
completeName = os.path.join(save_path, file_name)
os.chmod(completeName, stat.S_IWRITE)
print(completeName)
file1 = open(completeName, "w")
file1.write(setting_apex)
file1.close()
print(autoexecNAME)
file2 = open(autoexecNAME, "w")
file2.write(autoexec)
file2.close()
print(autoexecNAME_verf)
file3 = open(autoexecNAME_verf, "w")
file3.write(autoexec)
file3.close()
subprocess.run(["attrib", "+r", completeName])
file4 = open(ps1,"w")
file4.write(fr'''$WshShell = New-Object -comObject WScript.Shell
$Shortcut = $WshShell.CreateShortcut("$Home\Desktop\New Apex.lnk")
$Shortcut.TargetPath = "{apex_exe}"
$Shortcut.Arguments = "+exec autoexec.cfg -dev -preload -high +fps_max 0 -novid -fullscreen -dxlevel95"
$Shortcut.Save()''')
file4.close()

p = subprocess.Popen(f'powershell.exe -ExecutionPolicy RemoteSigned -file "{ps1}"')
p.communicate()
